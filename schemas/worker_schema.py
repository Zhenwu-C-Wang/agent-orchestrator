...
class ComparisonTaskInput(BaseModel):
    question: str
    context_files: list[str] = []
    context_urls: list[str] = []
    research: ResearchResult | None = None
...
class WriterTaskInput(BaseModel):
    ...
    def validate_intermediate_result(self) -> "WriterTaskInput":
        if self.research is None and self.analysis is None and self.comparison is None:
            raise ValueError("WriterTaskInput requires research, analysis, or comparison.")
        return self
...
class ReviewTaskInput(BaseModel):
    ...
    def validate_intermediate_result(self) -> "ReviewTaskInput":
        if self.research is None and self.analysis is None and self.comparison is None:
            raise ValueError("ReviewTaskInput requires research, analysis, or comparison.")
        return self
